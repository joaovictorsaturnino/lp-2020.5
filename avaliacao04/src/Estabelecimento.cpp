...
void Estabelecimento::listar(){
   cout << "cod, nome, preco, unidade de medida " << endl; 
    for(int i = 0; i < produtos.size(); i++){
        ...
        cout << produtos[i].unidadeMedida << endl;
    }
}

void Estabelecimento::completarProduto(Produto produto){
    for (int i = 0; i < produtos.size(); i++){
        if(produtos[i].nome.find(produto.nome) != string::npos){
            produto.codigo = produtos[i].codigo;
            produto.preco = produtos[i].preco;
            produto.quantidade = produtos[i].quantidade;
            produto.unidadeMedida = produtos[i].unidadeMedida;
        }
    }
}
